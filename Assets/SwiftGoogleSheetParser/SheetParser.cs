...
namespace SwiftGoogleSheetParser
{
    ...
    {
        static void SetField(FieldInfo field, object target, string txt, params ICustomStringParser[] stringParsers)
        {
            foreach (var customParser in stringParsers)
            {
                if (customParser.IsOfType(field.FieldType))
                {
                    object res;
        ...
        public static void ParseToList<T>(List<T> list, string[] rows, string separator, params ICustomStringParser[] stringParsers) where T : class, new()
        {
            var separatorChars = separator.ToCharArray();
